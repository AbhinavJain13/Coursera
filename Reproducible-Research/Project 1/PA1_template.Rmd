...
```
The mean of the total number of steps taken each day is `r round(mean.steps,0)`.

```{r}
median.steps <- median(total_steps,na.rm=TRUE)
```
The median of the total number of steps taken each day is `r median.steps`.
...
The 5 minute interval which have maximum number of average step across all days is `r names(maxsteps_interval)`, with `r round(maxsteps_interval[[1]],1)` number of steps.
...
Number of missing values is `r number_of_missing_values`
...
```
 ...
 a) Mean with missing value treated : `r round(mean.steps_new,0)`
 b) Mean without missing value treated : `r mean.steps`
 c) Median with missing value treated : `r round(median.steps_new,0)`
 d) Median without missing value treated : `r median.steps`
